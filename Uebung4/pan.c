...
{	int j, h = now._nr_pr;
	...
	{	int l_in;
		...
		{
			((P0 *)pptr(h))->_1_anwesendVorUeberfahrt[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			((P0 *)pptr(h))->_1_anwesendNachUeberfahrt[l_in] = 0;
		}
	}
		((P0 *)pptr(h))->_1_i = 0;
#ifdef VAR_RANGES
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("A:_1_anwesendVorUeberfahrt[l_in]", ((P0 *)pptr(h))->_1_anwesendVorUeberfahrt[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("A:_1_anwesendNachUeberfahrt[l_in]", ((P0 *)pptr(h))->_1_anwesendNachUeberfahrt[l_in]);
		}
	}
		logval("A:i", ((P0 *)pptr(h))->_1_i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	if ((Maxbody % WS) != 0)
		...
		now.man = 0;
#ifdef VAR_RANGES
		logval("goat", now.goat);
		logval("wolf", now.wolf);
		logval("cabbage", now.cabbage);
		logval("man", now.man);
#endif
}
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    anwesendVorUeberfahrt[%d]:	%d\n", l_in, ((P0 *)pptr(pid))->_1_anwesendVorUeberfahrt[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    anwesendNachUeberfahrt[%d]:	%d\n", l_in, ((P0 *)pptr(pid))->_1_anwesendNachUeberfahrt[l_in]);
		}
	}
	printf("	byte   i:	%d\n", ((P0 *)pptr(pid))->_1_i);
		break;
